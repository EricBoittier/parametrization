...
class PropertyRun:
    ...
    def init_properties(self):
        ...
        self.average_volume = np.mean(self.volumes, dtype=np.float64).to("angstrom cubed")
        ...
        self.Cp_SF = get_C_p_STANDARD_FLUCTUATIONS(self.avgH, self.avg_H_2, self.Nmol, self.avgTemp)
        self.dHvap = self.avgUgas - self.potential_per_molecule + self.avgTemp * Gas_const.to("kcal/(mol*K)")
...
def get_D(time, msd, skip=5):
    ...
    """Get self-diffusion coefficient by fitting a linear line to the MSD vs Time graph"""
...
def fill_property_dataclass(path, MW=34.02, nMol=202):
    """starting from the directory, get and fill all the required data for the dataclass"""
    #  paths
    ...
    gas_pots = []
    gas_temps = []
    outfiles = [x for x in os.listdir(gas_dyna_path) if x.startswith("job") and x.endswith("out")]
...
def get_gas_phase_results(path):
    """
    Read output file from gasphase simulation and return average energy and temperature as calculated by
    CHARMM...
    :param path:
    :return: average energy, average time
    """
    with open(path) as f:
        lines = [l for l in f.readlines() if l.startswith("AVER>")]
        # 3 vs 5
