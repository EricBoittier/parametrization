...
heat_capacity["cal mol K"] = (heat_capacity["J mol K"].to_numpy() * ureg("J/(mol*K)")).to("cal/(mol*K)")
...
def plot_properties(data_dfs, data_labels, FONTSIZE=20, show=False):
    ...
    ax_dict["F"].tick_params(axis='y', which='both', labelleft=False,
                             labelright=True)
    ax_dict["F"].plot(heat_capacity["K"], heat_capacity["cal mol K"], "-o", label="Exp.", markevery=2)
...
def get_params(df):
    ...
    lines_to_keep = [[l[0], float(l[1]), float(l[2]), float(l[3])] for l in lines_to_keep]
